...
import { getFeroniaStockData, getFeroniaBaseUrl } from '../services/feroniaService';
...
const FeroniaIntegration = () => {
  ...
  const checkConnection = useCallback(async () => {
    ...
    try {
      ...
      setConnectionStatus(Array.isArray(rows) ? 'connected' : 'error');
    } catch (_) {
        ...
        <section style={{ background: '#fff', borderRadius: 12, border: '1px solid #e2e8f0', overflow: 'hidden', marginBottom: 24 }}>
          ...
          {connectionStatus === 'error' && (
            <div style={{ padding: 12, margin: 12, background: '#fffbeb', borderRadius: 8, border: '1px solid #fcd34d', fontSize: 13, color: '#92400e' }}>
              <strong>Connection failed?</strong> Current source: <code style={{ background: '#fef3c7', padding: '2px 6px', borderRadius: 4 }}>{`${getFeroniaBaseUrl()}/getTAGIT_StockData`}</code>. In HTTPS production, configure same-origin proxy path <code style={{ background: '#fef3c7', padding: '2px 6px', borderRadius: 4 }}>/api/Feronia/getTAGIT_StockData</code>.
            </div>
          )}
        ...
        <div style={{ padding: 20, background: 'linear-gradient(135deg, #f5f3ff 0%, #ede9fe 100%)', borderRadius: 12, border: '1px solid #ddd6fe', display: 'flex', alignItems: 'flex-start', gap: 12 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: '#5b21b6', margin: 0, lineHeight: 1.5 }}>
              You are viewing this page as client <strong>{clientCode}</strong>. Feronia API: <strong>getTAGIT_StockData</strong>.
            </p>
          </div>
        </div>
      </div>
