...
const CustomerSidebarForm = ({ open, onClose, onSave }) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      /* Parent shows errors; keep sidebar open */
    }
  };
    ...
    <>
      ...
      >
        <div style={{ padding: '12px 14px', borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <h3 style={{ margin: 0, fontSize: 15, fontWeight: 800, color: '#0f172a' }}>Add Customer</h3>
        ...
        <div style={{ padding: 14, overflowY: 'auto', flex: 1 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 8 }}>
            <div><label style={labelStyle}>First Name *</label><input className="customer-sidebar-control" style={inputStyle} value={form.firstName} onChange={(e) => update('firstName', e.target.value)} /></div>
            <div><label style={labelStyle}>Last Name *</label><input className="customer-sidebar-control" style={inputStyle} value={form.lastName} onChange={(e) => update('lastName', e.target.value)} /></div>
            <div><label style={labelStyle}>Company</label><input className="customer-sidebar-control" style={inputStyle} value={form.companyName} onChange={(e) => update('companyName', e.target.value)} /></div>
            <div><label style={labelStyle}>Email</label><input className="customer-sidebar-control" style={inputStyle} type="email" value={form.email} onChange={(e) => update('email', e.target.value)} /></div>
            <div><label style={labelStyle}>Mobile *</label><input className="customer-sidebar-control" style={inputStyle} value={form.contactNumber} onChange={(e) => update('contactNumber', e.target.value)} /></div>
            <div><label style={labelStyle}>Aadhar</label><input className="customer-sidebar-control" style={inputStyle} value={form.aadharNumber} onChange={(e) => update('aadharNumber', e.target.value)} /></div>
            <div><label style={labelStyle}>PAN</label><input className="customer-sidebar-control" style={inputStyle} value={form.panNumber} onChange={(e) => update('panNumber', e.target.value)} /></div>
            <div style={{ gridColumn: '1 / -1' }}><label style={labelStyle}>Remarks</label><input className="customer-sidebar-control" style={inputStyle} value={form.remarks} onChange={(e) => update('remarks', e.target.value)} /></div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 8 }}>
            ...
            <input className="customer-sidebar-control" style={inputStyle} placeholder="Pincode" inputMode="numeric" maxLength={6} value={form.pincode} onChange={(e) => update('pincode', e.target.value.replace(/\D/g, '').slice(0, 6))} />
          </div>
        </div>
